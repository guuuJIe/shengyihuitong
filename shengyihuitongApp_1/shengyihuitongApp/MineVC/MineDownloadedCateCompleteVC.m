...
//    __weak typeof(self) weakSelf = self;
    ...
    self.title = self.lastDownModel.course_name;
...
- (void)initVideoList{
    ...
    NSString *where = [NSString stringWithFormat:@"where %@=%@ and %@=%@",bg_sqlKey(@"courseid"),bg_sqlValue(@(self.lastDownModel.course_id)),bg_sqlKey(@"isDownload"),bg_sqlValue(@1)];
    ...
    [datas enumerateObjectsUsingBlock:^(DownloadInfo  * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        for (PLVVodDownloadInfo *model in dbInfos) {
            if ([obj.identifier isEqualToString:model.identifier]) {
                ...
                model.chapter_name = obj.chapter_name;
                model.vid = obj.vid;
...
//     普通视频播放页面入口
     ...
     detailVC.competeArray = self.downloadInfos;
#else
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSString *delwhere = [NSString stringWithFormat:@"where %@=%@ and %@=%@",bg_sqlKey(@"identifier"),bg_sqlValue(localModel.identifier),bg_sqlKey(@"courseid"),bg_sqlValue(@(self.lastDownModel.course_id))];
    [DownloadInfo bg_delete:childDownloadTable where:delwhere];
    
    NSString *where = [NSString stringWithFormat:@"set %@=%@ where %@=%@ and %@=%@",bg_sqlKey(@"downloaded_num"),bg_sqlValue(@(self.downloadInfos.count)),bg_sqlKey(@"userPhone"),bg_sqlValue(userMobile),bg_sqlKey(@"course_id"),bg_sqlValue(@(self.lastDownModel.course_id))];

...
- (void)setupUI{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(getData) name:@"reloadStatues" object:nil];
...
- (void)selectVideoVid{
    [self.vidArray removeAllObjects];
//    dispatch_async(dispatch_get_global_queue(0, 0), ^{
    NSInteger index = 0;
        for (Chapter_list *chapter in self.courseModel.chapter_list) {
            for (int j = 0;j<chapter.child_list.count;j++) {
                Child_list *model = chapter.child_list[j];
                NSMutableDictionary *dic = [NSMutableDictionary dictionary];
                [dic setValue:@(index) forKey:@"index"];
                [dic setValue:model.video_id forKey:@"video_id"];
                [dic setValue:@(model.chapter_id) forKey:@"chapter_id"];
                [self.vidArray addObject:dic];
                index += 1;
            }
        }
//    });
...
//        [cell setupData:self.dataDic];
        ...
        if (!cell) {
            ...
            self.pageContentView.contentViewCurrentIndex = self.recordIndex;
...
- (NSMutableArray *)vidArray{
    if (!_vidArray) {
        _vidArray = [NSMutableArray array];
    }

...
- (void)initWithProperty
{
    ...
    self.titleSelectFont = [UIFont systemFontOfSize:15];
}
//重新布局frame
- (void)layoutSubviews
{
    ...
    [UIView animateWithDuration:(animated?0.1:0) animations:^{
        switch (self.indicatorType) {
            ...
            case FSIndicatorTypeCustom:
                self.indicatorView.center = CGPointMake(selectBtn.center.x, CGRectGetHeight(self.scrollView.bounds) - 5);
                self.indicatorView.bounds = CGRectMake(0, 0, indicatorWidth, 2);
                break;
...
- (void)setPageTitleViewWithProgress:(CGFloat)progress originalIndex:(NSInteger)originalIndex targetIndex:(NSInteger)targetIndex{
    ...
    UIButton *originalBtn = self.itemBtnArr[originalIndex];
    UIButton *targetBtn = self.itemBtnArr[targetIndex];
    
    [self P_indicatorScrollStyleHalfEndWithProgress:progress originalBtn:originalBtn targetBtn:targetBtn targetIndex:targetIndex];
}


#pragma mark - - - SGPageTitleView 滚动样式下指示器 SGIndicatorScrollStyleHalf 和 SGIndicatorScrollStyleEnd 滚动样式
- (void)P_indicatorScrollStyleHalfEndWithProgress:(CGFloat)progress originalBtn:(UIButton *)originalBtn targetBtn:(UIButton *)targetBtn targetIndex:(NSInteger)targetIndex{
    // 2、处理指示器下划线、遮盖样式
       if (progress == 1.0) {
...
//           CGFloat targetWidth = targetBtn.ag_centerX;
//           CGFloat orginWidth = originalBtn.ag_centerX;
//           CGFloat tempIndicatorWidth = self.configure.indicatorAdditionalWidth + tempSize.width;
//           [UIView animateWithDuration:0.1 animations:^{
//               if (tempIndicatorWidth >= originalBtn.ag_width) {
//                   self.indicatorView.ag_width = originalBtn.SG_width;
//               } else {
//                   self.indicatorView.ag_x = tempIndicatorWidth;
//               }
//               self.indicatorView.ag_x = (targetWidth - orginWidth) *progress+targetWidth;
//
//           }];
       }
}

...
+ (BOOL)insert:(NSString *)table keyValues:(NSDictionary *)keyValues replace:(BOOL)replace {
    ...
    NSMutableString * createSql = [NSMutableString stringWithFormat:@"create table if not exists '%@' (%@) ", table, [columns componentsJoinedByString:@","]];
    
    if ([self createTable:createSql]) {
        JLog(@"创表成功");
    }else{
        JLog(@"创表失败");
        return false;
